    ...
    public class OverallResultCalculationMapper : Profile
    {
        public OverallResultCalculationMapper()
        {
            ...
            CreateMap<OverallResult, DownloadOverallResultSlipsData>()
                ...
                .ForMember(d => d.CoreAssessmentSeries, opts => opts.MapFrom(s => s.TqRegistrationPathway.TqPathwayAssessments.Where(a => a.IsOptedin && a.EndDate == null).OrderByDescending(a => a.Id).FirstOrDefault().AssessmentSeries.Name))
                .ForMember(d => d.SpecialismAssessmentSeries, opts => opts.MapFrom(s => s.TqRegistrationPathway.TqRegistrationSpecialisms.FirstOrDefault(r => r.IsOptedin && r.EndDate == null).TqSpecialismAssessments.OrderByDescending(s => s.Id).FirstOrDefault(a => a.IsOptedin && a.EndDate == null).AssessmentSeries.Name))
                .ForMember(d => d.OverallResult, opts => opts.MapFrom(s => s.ResultAwarded))
                .ForMember(d => d.Details, opts => opts.MapFrom(s => JsonConvert.DeserializeObject<OverallResultDetail>(s.Details)))
