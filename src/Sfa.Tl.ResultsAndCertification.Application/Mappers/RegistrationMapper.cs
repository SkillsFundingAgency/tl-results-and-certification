    ...
    public class RegistrationMapper : Profile
    {
        public RegistrationMapper()
        {
            CreateMap<TqRegistrationPathway, RegistrationDetails>()
                ...
                .ForMember(d => d.IsPathwayAvailable, opts => opts.MapFrom(s => s.TqProvider.TqAwardingOrganisation.TlPathway.IsAvailable))
                .ForMember(d => d.EndYear, opts => opts.MapFrom(s => s.TqProvider.TqAwardingOrganisation.TlPathway.EndYear))
                .ForMember(d => d.IsActiveWithOtherAo, opts => opts.MapFrom((src, dest, destMember, context) => (bool)context.Items["IsActiveWithOtherAo"]))
                .ForMember(d => d.HasActiveAssessmentResults, opts => opts.MapFrom((src, dest, destMember, context) => (bool)context.Items["HasActiveAssessmentResults"]))
