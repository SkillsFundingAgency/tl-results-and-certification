...
{
    public class UploadWithdrawlsRequestViewModel
    {
        public long AoUkprn { get; set; }

        [DataType(DataType.Upload)]
        [Required(ErrorMessageResourceType = typeof(ErrorResource.UploadWithdrawls), ErrorMessageResourceName = "Select_File_To_Upload_Required_Validation_Message")]
        [FileValidation(AllowedExtensions = ".csv", MaxFileNameLength = 150, MaxFileSizeInMb = Constants.MaxFileSizeInMb, MaxRecordCount = 10000, ErrorResourceType = typeof(ErrorResource.UploadWithdrawls))]
        ...
        public void SetAnyModelErrors(ModelStateDictionary modelState)
        {
            ...
            {
                var error = ModelStateHelper.GetUploadErrorMessage((RequestErrorType)RequestErrorTypeId, typeof(ErrorResource.UploadWithdrawls));
