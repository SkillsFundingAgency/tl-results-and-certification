...
namespace Sfa.Tl.ResultsAndCertification.Web.ViewModel.Registration.Manual
{
    public class CheckAndSubmitViewModel
    {
        ...
        public List<string> GetSelectedSpecialisms => RegistrationModel.SelectSpecialisms?.PathwaySpecialisms.Specialisms.Where(x => x.IsSelected)
            .SelectMany(x => x.Code.Split(Constants.PipeSeperator)).Select(x => $"{RegistrationModel.SelectSpecialisms?.PathwaySpecialisms.SpecialismsLookup.FirstOrDefault(lkp => lkp.Key.Equals(x, System.StringComparison.InvariantCultureIgnoreCase)).Value} ({x})" ).ToList();
