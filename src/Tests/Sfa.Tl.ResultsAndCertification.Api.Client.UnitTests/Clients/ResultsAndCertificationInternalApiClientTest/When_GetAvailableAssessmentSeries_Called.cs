...
namespace Sfa.Tl.ResultsAndCertification.Api.Client.UnitTests.Clients.ResultsAndCertificationInternalApiClientTest
{
    public class When_GetAvailableAssessmentSeries_Called : BaseTest<ResultsAndCertificationInternalApiClient>
    {
        ...
        {
            HttpClient = new HttpClient(new MockHttpMessageHandler<AvailableAssessmentSeries>(_mockHttpResult, string.Format(ApiConstants.GetAvailableAssessmentSeriesUri, _ukprn, _profileId, (int)assessmentEntryType), HttpStatusCode.OK));
            _apiClient = new ResultsAndCertificationInternalApiClient(HttpClient, _tokenServiceClient, _configuration);
        }

        public async override Task When()
        {
            _result = await _apiClient.GetAvailableAssessmentSeriesAsync(_ukprn, _profileId, assessmentEntryType);
        }
