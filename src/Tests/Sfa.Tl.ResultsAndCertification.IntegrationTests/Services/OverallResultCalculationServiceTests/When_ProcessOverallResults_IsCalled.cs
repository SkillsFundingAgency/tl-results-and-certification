...
{
    public class When_ProcessOverallResults_IsCalled : OverallResultCalculationServiceBaseTest
    {
        ...
        public async Task WhenAsync(TqRegistrationPathway pathway)
        {
            ...
            var assessmentId = learnerPathways.FirstOrDefault().TqPathwayAssessments.FirstOrDefault().AssessmentSeriesId;
            var assessmentSeries = AssessmentSeries.FirstOrDefault(x => x.Id == assessmentId); 

            // TODO: Following action method can be public in order to do the testing?
            //await OverallResultCalculationService.ProcessOverallResults(learnerPathways.AsEnumerable(), TlLookup, OverallGradeLookup, assessmentSeries);
        }
