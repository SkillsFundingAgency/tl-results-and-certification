...
namespace Sfa.Tl.ResultsAndCertification.IntegrationTests.Services.AssessmentServiceTests
{
    ...
    public class When_CompareAndProcessAssessmentsAsync_Called_With_Valid_Specialism_Couplets : IClassFixture<BulkAssessmentsTextFixture>
    {
        ...
        private int _assessmentSeriesId;

        public When_CompareAndProcessAssessmentsAsync_Called_With_Valid_Specialism_Couplets(BulkAssessmentsTextFixture bulkAssessmentsTestFixture)
        {
            // Given
            ...
            _removeAssessmentEntryUln = 1111111111;
            var removeAssessmentEntryProfile = _bulkAssessmentsTestFixture.SeedRegistrationData(_removeAssessmentEntryUln, isCouplet: true);
            var removeAssessmentEntryPathway = _bulkAssessmentsTestFixture.SeedPathwayAssessmentData(removeAssessmentEntryProfile);

            foreach (var specialism in removeAssessmentEntryProfile.TqRegistrationPathways.SelectMany(x => x.TqRegistrationSpecialisms))
            {
                var removeAssessmentEntrySpecialism = _bulkAssessmentsTestFixture.SeedSpecialismAssessmentData(specialism);
                _assessmentSeriesId = removeAssessmentEntrySpecialism.AssessmentSeriesId;
            }
            
            // Seed Profile data
            _bulkAssessmentsTestFixture.Ulns = new List<long> { 1111111112 };

            var registrations = new List<TqRegistrationProfile>();

            foreach (var uln in _bulkAssessmentsTestFixture.Ulns)
                registrations.Add(_bulkAssessmentsTestFixture.SeedRegistrationData(uln, isCouplet: true));

            _bulkAssessmentsTestFixture.TqRegistrationProfilesData = registrations;
        ...
        public async Task Then_Expected_Assessments_Are_Created()
        {
            ...
            foreach (var uln in _bulkAssessmentsTestFixture.Ulns)
            {
                ...
                var registrationSpecialisms = registrationPathway.TqRegistrationSpecialisms;

                foreach(var registrationSpecialism in registrationSpecialisms)
                {
            ...
            actualRemovedPathwayAssessment.AssessmentSeriesId.Should().Be(_assessmentSeriesId);
            ...
            foreach (var removeRegistrationSpecialism in removeRegistrationPathway.TqRegistrationSpecialisms)
            {
                foreach (var expectedRemovedSpecialismAssessment in _bulkAssessmentsTestFixture.TqSpecialismAssessmentsData.Where(p => p.TqRegistrationSpecialismId == removeRegistrationSpecialism.Id))
                {
                    var actualRemovedSpecialismAssessment = _bulkAssessmentsTestFixture.DbContext.TqSpecialismAssessment.FirstOrDefault(x => x.TqRegistrationSpecialismId == removeRegistrationSpecialism.Id);
                    ...
                    actualRemovedSpecialismAssessment.AssessmentSeriesId.Should().Be(_assessmentSeriesId);
