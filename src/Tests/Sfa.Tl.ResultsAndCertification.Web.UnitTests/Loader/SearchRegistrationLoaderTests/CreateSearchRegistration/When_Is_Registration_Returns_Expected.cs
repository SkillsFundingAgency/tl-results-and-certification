...
{
    public class When_Is_Registration_Returns_Expected : TestSetup
    {
        public override void Given()
            => Given(SearchRegistrationType.Registration);
        ...
        {
            Result.SearchType.Should().Be(SearchRegistrationType.Registration);
            Result.Criteria.Filters.Should().BeEquivalentTo(FiltersViewModel);

            Result.State.Should().Be(SearchRegistrationState.NoSearch);
            Result.PageTitle.Should().Be(SearchRegistration.Page_Title_Search_Registration);
            Result.PageHeading.Should().Be(SearchRegistration.Heading_Search_Registration);
            ...
            Result.Breadcrumb.BreadcrumbItems[1].DisplayName.Should().Be(Breadcrumb.Registration_Dashboard);
            Result.Breadcrumb.BreadcrumbItems[1].RouteName.Should().Be(RouteConstants.RegistrationDashboard);
            Result.Breadcrumb.BreadcrumbItems[1].RouteAttributes.Should().BeEmpty();

            Result.Breadcrumb.BreadcrumbItems[2].DisplayName.Should().Be(Breadcrumb.Search_For_Registration_Entry);
