...
namespace Sfa.Tl.ResultsAndCertification.Web.UnitTests.Loader.AdminPostResultsLoaderTests.GetAdminOpenSpecialismAppeal
{
    public abstract class TestSetup : AdminPostResultsLoaderBaseTest
    {
        ...
        protected AdminOpenSpecialismAppealViewModel Result { get; private set; }

        public override async Task When()
        {
            Result = await Loader.GetAdminOpenSpecialismAppealAsync(RegistrationPathwayId, SpecialismAssessmentId);
        }
        ...
        protected void AssertResult(AdminLearnerRecord apiResult)
        {
            ...
            Result.SummaryLearner.Id.Should().Be(AdminOpenSpecialismAppeal.Summary_Learner_Id);
            Result.SummaryLearner.Title.Should().Be(AdminOpenSpecialismAppeal.Summary_Learner_Text);
            Result.SummaryLearner.Value.Should().Be(Result.Learner);

            Result.SummaryUln.Id.Should().Be(AdminOpenSpecialismAppeal.Summary_ULN_Id);
            Result.SummaryUln.Title.Should().Be(AdminOpenSpecialismAppeal.Summary_ULN_Text);
            Result.SummaryUln.Value.Should().Be(Result.Uln.ToString());

            Result.SummaryProvider.Id.Should().Be(AdminOpenSpecialismAppeal.Summary_Provider_Id);
            Result.SummaryProvider.Title.Should().Be(AdminOpenSpecialismAppeal.Summary_Provider_Text);
            Result.SummaryProvider.Value.Should().Be(Result.Provider);

            Result.SummaryTlevel.Id.Should().Be(AdminOpenSpecialismAppeal.Summary_TLevel_Id);
            Result.SummaryTlevel.Title.Should().Be(AdminOpenSpecialismAppeal.Summary_TLevel_Text);
            Result.SummaryTlevel.Value.Should().Be(Result.Tlevel);

            Result.SummaryStartYear.Id.Should().Be(AdminOpenSpecialismAppeal.Summary_StartYear_Id);
            Result.SummaryStartYear.Title.Should().Be(AdminOpenSpecialismAppeal.Summary_StartYear_Text);
            ...
            Result.SummaryExamPeriod.Id.Should().Be(AdminOpenSpecialismAppeal.Summary_Exam_Period_Id);
            Result.SummaryExamPeriod.Title.Should().Be(AdminOpenSpecialismAppeal.Summary_Exam_Period_Text);
            Result.SummaryExamPeriod.Value.Should().Be(Result.ExamPeriod);

            Result.SummaryGrade.Id.Should().Be(AdminOpenSpecialismAppeal.Summary_Grade_Id);
            Result.SummaryGrade.Title.Should().Be(AdminOpenSpecialismAppeal.Summary_Grade_Text);
