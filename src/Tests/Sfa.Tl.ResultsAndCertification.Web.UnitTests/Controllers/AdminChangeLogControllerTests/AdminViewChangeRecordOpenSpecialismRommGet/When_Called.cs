...
namespace Sfa.Tl.ResultsAndCertification.Web.UnitTests.Controllers.AdminChangeLogControllerTests.AdminViewChangeRecordOpenSpecialismRommGet
{
    public class When_Called : AdminChangeLogControllerTestBase
    {
        private IActionResult Result;

        private AdminViewChangeRecordOpenSpecialismRommViewModel Mockresult;

        public override void Given()
        {
            Mockresult = CreateViewModel();

            AdminChangeLogLoader.GetAdminViewChangeOpenSpecialismRommRecord(ChangeLogId).Returns(Mockresult);
        }

        public override async Task When()
            => Result = await Controller.AdminViewChangeRecordOpenSpecialismRommAsync(ChangeLogId);

        [Fact]
        public void Then_Expected_Methods_AreCalled()
        {
            AdminChangeLogLoader.Received(1).GetAdminViewChangeOpenSpecialismRommRecord(ChangeLogId);
        }
        ...
        public void Then_Return_Expected_Results()
        {
            ...
            var model = (Result as ViewResult).Model as AdminViewChangeRecordOpenSpecialismRommViewModel;
        ...
        protected AdminViewChangeRecordOpenSpecialismRommViewModel CreateViewModel() => new()
        {
            ...
            ChangeType = Common.Enum.ChangeType.OpenPathwayRomm
        };
    }
}