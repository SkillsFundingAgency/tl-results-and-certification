...
namespace Sfa.Tl.ResultsAndCertification.Web.UnitTests.Controllers.AdminChangeLogControllerTests.AdminViewChangeRecordOpenSpecialismAppealGet
{
    public class When_Called : AdminChangeLogControllerTestBase
    {
        private IActionResult Result;

        private AdminViewChangeRecordOpenSpecialismAppealViewModel Mockresult;

        public override void Given()
        {
            Mockresult = CreateViewModel();

            AdminChangeLogLoader.GetAdminViewChangeOpenSpecialismAppealRecord(ChangeLogId).Returns(Mockresult);
        }

        public override async Task When()
            => Result = await Controller.AdminViewChangeRecordOpenSpecialismAppealAsync(ChangeLogId);

        [Fact]
        public void Then_Expected_Methods_AreCalled()
        {
            AdminChangeLogLoader.Received(1).GetAdminViewChangeOpenSpecialismAppealRecord(ChangeLogId);
        }
        ...
        public void Then_Return_Expected_Results()
        {
            ...
            var model = (Result as ViewResult).Model as AdminViewChangeRecordOpenSpecialismAppealViewModel;
        ...
        protected AdminViewChangeRecordOpenSpecialismAppealViewModel CreateViewModel() => new()
        {
            ...
            ChangeType = Common.Enum.ChangeType.OpenSpecialismAppeal
        };
    }
}