...
namespace Sfa.Tl.ResultsAndCertification.Web.UnitTests.Controllers.AdminChangeLogControllerTests.AdminViewChangeRecordOpenPathwayRommGet
{
    public class When_Called : AdminChangeLogControllerTestBase
    {
        private IActionResult Result;

        private AdminViewChangeRecordOpenPathwayRommViewModel Mockresult;

        public override void Given()
        {
            Mockresult = CreateViewModel();

            AdminChangeLogLoader.GetAdminViewChangeOpenPathwayRommRecord(ChangeLogId).Returns(Mockresult);
        }

        public override async Task When()
            => Result = await Controller.AdminViewChangeRecordOpenPathwayRommAsync(ChangeLogId);

        [Fact]
        public void Then_Expected_Methods_AreCalled()
        {
            AdminChangeLogLoader.Received(1).GetAdminViewChangeOpenPathwayRommRecord(ChangeLogId);
        }
        ...
        public void Then_Return_Expected_Results()
        {
            ...
            var model = (Result as ViewResult).Model as AdminViewChangeRecordOpenPathwayRommViewModel;
        ...
        protected AdminViewChangeRecordOpenPathwayRommViewModel CreateViewModel() => new()
        {
