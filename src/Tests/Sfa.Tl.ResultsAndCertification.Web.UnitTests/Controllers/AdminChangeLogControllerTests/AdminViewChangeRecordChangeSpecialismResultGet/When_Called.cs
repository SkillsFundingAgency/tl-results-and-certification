...
namespace Sfa.Tl.ResultsAndCertification.Web.UnitTests.Controllers.AdminChangeLogControllerTests.AdminViewChangeRecordChangeSpecialismResultGet
{
    public class When_Called : AdminChangeLogControllerTestBase
    {
        private IActionResult Result;

        private AdminViewChangeRecordSpecialismResultViewModel Mockresult;
        private ChangeSpecialismDetails details;

        public override void Given()
        {
            details = new()
            {
                SpecialismAssessmentId = 3137581,
                SpecialismResultId = 2888360,
                SelectedGradeFrom = "Distinction",
                SelectedGradeTo = "X - no result"
            };

            Mockresult = CreateViewModel();

            AdminChangeLogLoader.GetAdminViewChangeSpecialismResultRecord(ChangeLogId).Returns(Mockresult);
        }

        public override async Task When()
            => Result = await Controller.AdminViewChangeRecordChangeSpecialismResultAsync(ChangeLogId);

        [Fact]
        public void Then_Expected_Methods_AreCalled()
        {
            AdminChangeLogLoader.Received(1).GetAdminViewChangeSpecialismResultRecord(ChangeLogId);
        }
        ...
        public void Then_Return_Expected_Results()
        {
            ...
            var model = (Result as ViewResult).Model as AdminViewChangeRecordSpecialismResultViewModel;
            ...
            model.SpecialismDetails.SpecialismResultId.Should().Be(Mockresult.SpecialismDetails.SpecialismResultId);
            model.SpecialismDetails.SelectedGradeFrom.Should().Be(Mockresult.SpecialismDetails.SelectedGradeFrom);
            model.SpecialismDetails.SelectedGradeTo.Should().Be(Mockresult.SpecialismDetails.SelectedGradeTo);
        ...
        protected AdminViewChangeRecordSpecialismResultViewModel CreateViewModel() => new()
        {
