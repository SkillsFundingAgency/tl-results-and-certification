...

{
    public abstract class TestSetup : AdminDashboardControllerTestBase
    {
        ...
        {
            Result = await Controller.AdminAddRommOutcomeSpecialismAsync(RegistrationPathwayId, AssessmentId);
        }

        protected static AdminAddRommOutcomeSpecialismViewModel ViewModel
            => new()
            {
