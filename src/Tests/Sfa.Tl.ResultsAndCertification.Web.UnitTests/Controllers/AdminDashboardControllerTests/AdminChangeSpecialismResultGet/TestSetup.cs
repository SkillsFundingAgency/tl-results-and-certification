...
namespace Sfa.Tl.ResultsAndCertification.Web.UnitTests.Controllers.AdminDashboardControllerTests.AdminChangeSpecialismResultGet
{
    public abstract class TestSetup : AdminDashboardControllerTestBase
    {
        ...
        {
            Result = await Controller.AdminChangeSpecialismResultAsync(RegistrationPathwayId, AssessmentId);
        }

        protected static AdminChangeSpecialismResultViewModel ViewModel
            => new()
            {
