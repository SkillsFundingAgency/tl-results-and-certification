...

{
    public abstract class TestSetup : AdminPostResultsControllerTestBase
    {
        ...
        {
            Result = await Controller.AdminReviewChangesAppealOutcomeSpecialismAsync();
        }

        protected static AdminAddAppealOutcomeChangeGradeSpecialismViewModel CreateViewModel()
            => new()
            {
