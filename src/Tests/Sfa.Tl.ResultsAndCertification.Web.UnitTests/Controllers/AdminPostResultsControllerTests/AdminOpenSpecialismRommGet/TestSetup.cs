...
namespace Sfa.Tl.ResultsAndCertification.Web.UnitTests.Controllers.AdminPostResultsControllerTests.AdminOpenSpecialismRommGet
{
    public abstract class TestSetup : AdminPostResultsControllerTestBase
    {
        ...
        {
            Result = await Controller.AdminOpenSpecialismRommAsync(RegistrationPathwayId, SpecialismAssessmentId);
        }

        protected static AdminOpenSpecialismRommViewModel CreateViewModel()
            => new()
            {
