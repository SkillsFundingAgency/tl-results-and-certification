...
namespace Sfa.Tl.ResultsAndCertification.Web.UnitTests.Controllers.ManageRegistrationControllerTests.ChangeCoreQuestionGet
{
    public abstract class TestSetup : BaseTest<ManageRegistrationController>
    {
        ...
        public override void Setup()
        {
            ...
            CacheKey = string.Concat(CacheKeyHelper.GetCacheKey(httpContext.User.GetUserId(), CacheConstants.RegistrationCacheKey), Common.Helpers.Constants.ChangeRegistrationCoreProviderDetailsViewModel);
        }

        public async override Task When()
        {
            Result = await Controller.ChangeCoreQuestionAsync(ProfileId);
        }
    }
}
