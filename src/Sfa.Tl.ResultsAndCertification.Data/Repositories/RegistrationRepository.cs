...
namespace Sfa.Tl.ResultsAndCertification.Data.Repositories
{
    public class RegistrationRepository : GenericRepository<TqRegistrationProfile>, IRegistrationRepository
    {
        ...
        public async Task<TqRegistrationPathway> GetRegistrationLiteByProviderUkprnAsync(long providerUkprn, int profileId, bool includeProfile = true, bool includeIndustryPlacements = false, bool includeOverallResults = false)
        {
            var pathwayQueryable = _dbContext.TqRegistrationPathway.AsQueryable();
            ...
            var registrationPathway = await pathwayQueryable
               ...
               .Include(p => p.TqPathwayAssessments.Where(pa => pa.IsOptedin && (pa.TqRegistrationPathway.Status == RegistrationPathwayStatus.Withdrawn) ? pa.EndDate != null : pa.EndDate == null))
                    .ThenInclude(pa => pa.TqPathwayResults.Where(pr => pr.IsOptedin && pr.TqPathwayAssessment.TqRegistrationPathway.Status == RegistrationPathwayStatus.Withdrawn ? pr.EndDate != null : pr.EndDate == null))
               .OrderByDescending(p => p.CreatedOn)
               .FirstOrDefaultAsync(p => p.TqRegistrationProfile.Id == profileId && p.TqProvider.TlProvider.UkPrn == providerUkprn);
        ...
        public async Task<IList<TqRegistrationPathway>> GetRegistrationPathways()
        {
            var query = _dbContext.TqRegistrationPathway
                            ...
                            .Include(p => p.TqPathwayAssessments.Where(p => p.IsOptedin && p.EndDate == null))
                            ...
                            .Include(p => p.TqRegistrationSpecialisms
                                ...
                                .ThenInclude(s => s.TqSpecialismAssessments.Where(s => s.IsOptedin && s.EndDate == null))
        ...
        public async Task<IList<TqRegistrationSpecialism>> GetSpecialismRegistrationPathwaysByAssesmentSeriesYear(string[] assesmentSeriesYears)
        {
            var query = _dbContext.TqRegistrationSpecialism
                .Include(p => p.TlSpecialism)
                    .ThenInclude(p => p.TlDualSpecialismToSpecialisms)
                    .ThenInclude(p => p.DualSpecialism)
                .Include(p => p.TqSpecialismAssessments)
                   .ThenInclude(p => p.AssessmentSeries)
                ...
                .Include(p => p.TqSpecialismAssessments)
                    .ThenInclude(p => p.TqSpecialismResults)
                    .ThenInclude(p => p.TlLookup)
            .Where(p => p.TqSpecialismAssessments != null
